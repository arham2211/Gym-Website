import React from 'react'

export default function ChoseUs() {
    return (
        <section id="section4" className="my-4 pt-3" >
            <div className="container">
                <div className="row">
                    <div className="col-12 text-center my-5">
                        <h6 className='my-3 fw-bold'>WHY CHOSE US?</h6>
                        <h2 className='display-6'>PUSH YOUR LIMITS FORWARD</h2>
                    </div>
                </div>

                <div className="row">
                    <div id="modern" className="col-3 d-flex align-self-baseline align-items-center justify-content-center flex-column text-center py-3">
                        <div className='rounded-circle mb-4'><h1><i className="fa-solid fa-curling-stone"></i></h1></div>
                        <h3 className='mb-3'>Modern Equipment</h3>
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut dolore facilisis.</p>

                    </div>
                    <div id="healthy" className="col-3 d-flex align-self-baseline align-items-center justify-content-center flex-column text-center py-3">
                        <div className='rounded-circle mb-4'><h1><i className="fa-sharp fa-solid fa-glass-citrus"></i></h1></div>
                        <h3 className='mb-3'>Healthy Nutrition Plan</h3>
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut dolore facilisis.</p>

                    </div>
                    <div id="train" className="col-3 d-flex align-self-baseline align-items-center justify-content-center flex-column text-center py-3">
                        <div className='rounded-circle mb-4'><h1><i className="fa-sharp fa-solid fa-dumbbell"></i></h1></div>
                        <h3 className='mb-3'>Profesional training</h3>
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut dolore facilisis.</p>

                    </div>
                    <div id="needs" className="col-3 d-flex align-self-baseline align-items-center justify-content-center flex-column text-center py-3">
                        <div className='rounded-circle mb-4'><h1><i className="fa-solid fa-heart-pulse"></i></h1></div>
                        <h3 className='mb-3'>Unique to your needs</h3>
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut dolore facilisis.</p>

                    </div>
                   
                </div>
            </div>
        </section>
    )
}
